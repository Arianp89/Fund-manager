        ...
        with open('excel.csv' , 'r' , encoding='utf-8-sig') as f:
            for num in range(self.line_number):
                ...
                if file_list[0] != '':
                    ...
                    status = 'true'
                    file_data = {'full_name': file_list[-1].replace('\n','') + '' + file_list[-2],
                            'loan_amount': None,
                            'loan_number': None, 
                            ...
                            'total_capital': None,
                            'status':status}
                elif status == 'false' and file_list[0] != '':
                    file_data = {'full_name': file_list[-1].replace('\n','') + '' + file_list[-2],
                                'loan_amount': int(file_list[-3]),
                                'loan_number': int(file_list[-4]), 
                                'capital_increase': int(file_list[-5]),
                                'total_amount': int(file_list[-6]),
                                'total_capital': int(file_list[-7]),
                                'status':status}
